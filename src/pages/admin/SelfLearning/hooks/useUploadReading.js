...
const useUploadReading = ({ getJPTranslatedForAlert, getJPTranslatedMessages }) => {
    ...
    dispatch(
      ...
      .then(({ payload }) => {
        ...
        const customMessages = {
          'sheet-invalid':
            'エクセルファイルはParentシート、Chunksシート、Wordsシートを含んでいる必要があります。',
          'sheet-order':
            'シートの順番が適切ではありません。(Parent、Chunks、Wordsの順番である必要があります。)',
        };

        setErrors(getJPTranslatedMessages(data, customMessages));
...
export default useUploadReading;
