...
import { 
  ...
  phrase, 
  getCheckListIndex, 
} from '../../../../../../redux/checklist/selectors';
import { 
  setCheckListIndex,
  incrementCheckListIndex,
  decrementCheckListIndex,
  fetchChecklist,
} from '../../../../../../redux/checklist/slice';
...
const Commentary = () => {
  ...
  const { id: commentaryId, category } = useParams();
  const worResult = useSelector(word);
  const phraseResult = useSelector(phrase);
  const results = category === 'word' ? worResult : phraseResult;
  const resultDetailIndex = useSelector(getCheckListIndex);
  ...
  const handleNext = () => {
    if (resultDetailIndex < results.length - 1) {
      stop();
      dispatch(incrementCheckListIndex());
    }
  };
  ...
  const handlePrev = () => {
    if (resultDetailIndex > 0) {
      stop();
      dispatch(decrementCheckListIndex());
    }
  };
  ...
  const getAudioUrl = (fileName) => {
    return category
      ? `${process.env.REACT_APP_SERVER_API}/api/question-audio/${resultDetail.training_vocabularyable.training_unit_id}/${category}/${fileName}`
      : '';
  };

  useEffect(()=>{
    localStorage.setItem('previous_screen',`/proficiency/knowledge/check-list/${category}`);
  });
  ...
  useEffect(() => {
    if (resultDetail) {
      window.scrollTo(0, 0);
      history.replace(
        `/training/muscle-courses/check-list/commentary/${resultDetail.id}/${category}`
      );

      setExampleSentence(
        category == 'word'
          ? questionHelper.formatQuestionSentence(
              resultDetail.training_vocabularyable
                .example_sentence
            )
          : ''
      );
      ...
      setPronunciationTips(resultDetail.training_vocabularyable.pronunciation_point);

      setAudioUrl(
        new Audio(getAudioUrl(resultDetail.training_vocabularyable.audio_file))
      );

      if (category === 'word') {
        setSentenceAudioUrl(
          new Audio(getAudioUrl(resultDetail.training_vocabularyable.example_sentence_audio_file))
        );
      }
  ...
  useEffect(() => {
    if (results.length < 1) {
      ...
      dispatch(
        setCheckListIndex({ commentaryId, category })
      );
    }
  }, [results]);
  ...
  useEffect(() => {
    if (resultDetailIndex > -1) {
      setResultDetail(results[resultDetailIndex]);
      setIsFetchingData(false);
    }
  },[resultDetailIndex])
          ...
          <div className="flex-1">
            <div
              className={`px-px-24 -mt-px-15 ${
                category === 'word' ? `text-center` : `text-left`
              } pt-px-100`}
            >
              <p
              ...
              >
                {removeSpecialCharacters(
                  resultDetail.training_vocabularyable.title
                )}{' '}
              </p>
              <p className="font-theme-norm text-18 mt-px-8">
                {' '}
                {resultDetail.training_vocabularyable.meaning
                  ? resultDetail.training_vocabularyable
                      .meaning
                  : resultDetail.training_vocabularyable
                      .translation}{' '}
