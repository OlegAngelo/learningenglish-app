...
import { checkAnswer, isIncorrectEnItemChoice } from '../../../../../../utils/validationHelper';
import { QuestionWrapperContext } from '../../components/QuestionWrapper/QuestionWrapper';

import styles from './AudioEnglishWordSelectionQuestion.module.css';

const AudioEnglishWordSelectionQuestion = () => {
  const {
    footerProps,
    setFooterProps,
    isShowCommentary,
  ...
  const jpTranslation = questionItem.choices.filter((choice) => choice.is_correct)[0].jp_item;
  ...
  const handleAnswerOptionClick = ({ id, en_item, is_correct }) => {
    if (selected) return;
  ...
  const sessionQuestion = () => {
    if (learningType.training_word_id) {
      return "音声を聞いて、読まれた単語を選択しましょう";
    } else if (learningType.training_phrase_id) {
      return "音声を聞いて、読まれた英語を選択しましょう";
    }
  };
      ...
      {isShowCommentary
        ? (
          <div className="mx-px-8 pb-12">
            ...
            {
              pronuncationTips && (
                <div className="p-px-16 bg-basic-400 rounded">
            ...
            <div>
              ...
              ? (
                <Fragment>
                  <QuestionnaireFeedback
                    bottomText={translation}
                    topText={jpTranslation}
                    answer={selected}
                    response={response}
              ...
              {
                isAudioEnded && (
                  <div className={`mx-px-8 ${response && styles.responseChoices}`}>
...
export default AudioEnglishWordSelectionQuestion;
